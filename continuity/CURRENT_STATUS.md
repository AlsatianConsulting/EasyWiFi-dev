...
EasyWiFi is currently a working Rust/GTK desktop application with three mature areas and one expanding area:
...
26. Satcom parser supports protocol/decoder denylist filtering via `EASYWIFI_SATCOM_PARSE_DENYLIST` (comma-separated tokens), yielding `denied_by_policy` parse state without disabling full satcom observation.
...
130. `Presets -> Frequencies` now includes `Export Cellular ARFCN Playlist (CSV + JSON)`, emitting the full uplink/download ARFCN/UARFCN/EARFCN catalog for external use.
131. Cellular ARFCN playlist groups are now frequency-sorted before menu/render/export paths, fixing E-GSM mixed-range ordering and improving deterministic artifact output.
132. SDR satcom summary JSON and SDR health snapshot JSON now include `artifact_contract_version` for downstream schema pinning/compat checks.
...
3. `packaging/easywifi.desktop` exists.
