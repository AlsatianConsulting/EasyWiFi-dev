use anyhow::Result;
use simplestg::capture;
use simplestg::privilege::{HelperRequest, HelperResponse};
...
fn main() -> Result<()> {
    ...
    match args.next().as_deref() {
        ...
        _ => {
            eprintln!("usage: simplestg-helper daemon | hop <iface> <dwell_ms> <ht_mode> <channel...> | tshark <args...>");
            std::process::exit(2);
        }
    }
}
...
fn start_origin_parent_watchdog() {
    let Some(parent_pid) = std::env::var("SIMPLESTG_PARENT_PID")
        .ok()
...
fn run_channel_hop(args: Vec<String>) -> Result<()> {
    if args.len() < 4 {
        anyhow::bail!("usage: simplestg-helper hop <iface> <dwell_ms> <ht_mode> <channel...>");
    }
